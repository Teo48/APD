	...
	if (rank == MASTER) { // This code is run by a single process
		...
		*/
		...
		for (int i = 0; i < polynomialSize; i++) {
			...
			*/
			if (i) {
				MPI_Send(&a[i], 1, MPI_FLOAT, i, MPI_TAG_UB, MPI_COMM_WORLD);
			}
		}
		...
		MPI_Send(&x, 1, MPI_INT, rank + 1, MPI_TAG_UB, MPI_COMM_WORLD);
		MPI_Send(&a[0], 1, MPI_FLOAT, rank + 1, MPI_TAG_UB, MPI_COMM_WORLD);
	} else {
		float val, sum;
		int x;
		...
		*/
		MPI_Recv(&val, 1, MPI_FLOAT, MASTER, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(&x, 1, MPI_INT, rank - 1, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(&sum, 1, MPI_FLOAT, rank - 1, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		...
		if (rank == nProcesses - 1) {
			...
			MPI_Send(&x, 1, MPI_INT, rank + 1, MPI_TAG_UB, MPI_COMM_WORLD);
			MPI_Send(&sum, 1, MPI_FLOAT, rank + 1, MPI_TAG_UB, MPI_COMM_WORLD);
